...
@Serializable
data class TileData(val id : String, val image : String, val tile_areas : String, val count : Int)

@Serializable
data class TileOptions(val starting_tiles : Set<String>, val has_shield : Set<String>)

@Serializable
data class TileSet(val tiles : Array<TileData>, val tile_options : TileOptions)
...
class GameTilesLoader : IGameTilesLoader {
    ...
    private fun parseJsonTile(tileData: TileData, tileOptions: TileOptions, ) : Pair<Tile, Int> {
        ...
        val tileObjects = Array(TILE_AREA_SAMPLES_TOTAL) { i->
            /* Skip spaces */
