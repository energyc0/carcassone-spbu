...
internal class TileTest {
    companion object {
        val tileLookData = Array(TILE_AREA_SAMPLES_TOTAL) { i->
            when(i/5) {
    ...
    fun initTest() {
        assertDoesNotThrow {
            val tile1 = Tile(true, TileLook(tileLookData))
            val tile2 = Tile(false, TileLook(tileLookData))
    ...
    fun rotatedLookTest() {
        val tile = Tile(false, TileLook(tileLookData, Rotation.LEFT))
    ...
    fun edgesTest() {
        val tile = Tile(false, TileLook(tileLookData, Rotation.STRAIGHT))
